...
const sx = (nx) => STAGE_X + (nx / 100) * STAGE_W;
const sy = (ny) => STAGE_Y + (ny / 100) * STAGE_H;
const wpx = (w) => (w / 100) * STAGE_W; // normalized width → px
const hpx = (h) => (h / 100) * STAGE_H; // normalized height → px
...
// ── Camera ───────────────────────────────────────────────────────────────────
function cameraAt(spec, t) {
    var _a, _b, _c, _d, _e, _f;
    const keys = spec.camera && spec.camera.length
        ? spec.camera
        : [{ at: 0, center: [50, 50], zoom: 1 }];
    let cur = keys[0];
    let nxt = keys[0];
    for (let i = 0; i < keys.length; i++) {
        if (t >= keys[i].at) {
            cur = keys[i];
            nxt = (_a = keys[i + 1]) !== null && _a !== void 0 ? _a : keys[i];
        }
    }
    const c0 = (_b = cur.center) !== null && _b !== void 0 ? _b : [50, 50], c1 = (_c = nxt.center) !== null && _c !== void 0 ? _c : c0;
    const z0 = (_d = cur.zoom) !== null && _d !== void 0 ? _d : 1, z1 = (_e = nxt.zoom) !== null && _e !== void 0 ? _e : z0;
    const dur = (_f = nxt.dur) !== null && _f !== void 0 ? _f : 1.5;
...
function screenOf(nx, ny, cam) {
    return {
        x: SCREEN_CX + cam.zoom * (sx(nx) - sx(cam.cx)),
        y: SCREEN_CY + cam.zoom * (sy(ny) - sy(cam.cy)),
    };
}
function actorStateAt(spec, a, t) {
    var _a, _b, _c, _d;
    const home = (_a = a.at) !== null && _a !== void 0 ? _a : [50, 50];
    ...
    for (const e of evs) {
        const dur = (_b = e.dur) !== null && _b !== void 0 ? _b : 1.5;
        const p = ease((t - e.at) / dur);
        if (t < e.at)
            continue;
        if (e.action === 'appear')
            opacity = lerp(opacity, 1, p);
        else if (e.action === 'disappear')
            opacity = lerp(1, 0, p);
    ...
    let scale = (_c = a.size) !== null && _c !== void 0 ? _c : 1;
    for (const e of spec.events.filter((ev) => ev.actor === a.id && ev.action === 'pulse')) {
        const dur = (_d = e.dur) !== null && _d !== void 0 ? _d : 1;
...
function actorFx(spec, a, t) {
    var _a, _b;
    let rotate = (_a = a.rotate) !== null && _a !== void 0 ? _a : 0;
    let flow = 0;
    let fill = 0;
    for (const e of spec.events.filter((ev) => ev.actor === a.id)) {
        ...
        const dur = (_b = e.dur) !== null && _b !== void 0 ? _b : 1.5;
        if (e.action === 'rotate')
            rotate += (t - e.at) * 150; // ~150°/s spin while/after active
        else if (e.action === 'flow')
            flow = ((t - e.at) * 0.6) % 1; // dash offset 0..1
        else if (e.action === 'fill')
            fill = clamp((t - e.at) / dur); // 0 → 1 level
    }
...
function progress(e, t) {
    ...
    if (!e)
        return 0;
    return clamp((t - e.at) / ((_a = e.dur) !== null && _a !== void 0 ? _a : 1.5));
}
function procAt(spec, t) {
    var _a, _b, _c, _d;
    const find = (act) => spec.events.find((e) => e.action === act);
    ...
    const targetX = ((_c = (_b = (_a = cutE === null || cutE === void 0 ? void 0 : cutE.at_x) !== null && _a !== void 0 ? _a : hybridE === null || hybridE === void 0 ? void 0 : hybridE.at_x) !== null && _b !== void 0 ? _b : unwindE === null || unwindE === void 0 ? void 0 : unwindE.at_x) !== null && _c !== void 0 ? _c : 50);
    ...
    return {
        ...
        repairMode: (_d = repairE === null || repairE === void 0 ? void 0 : repairE.mode) !== null && _d !== void 0 ? _d : 'generic',
...
// ── Captions ─────────────────────────────────────────────────────────────────
export function captionAt(spec, t) {
    let cur;
    for (const e of spec.events)
        if (e.caption && t >= e.at && (!cur || e.at >= cur.at))
            cur = e;
    if (!cur)
        return { text: '', alpha: 0 };
...
// ── Scene drawing ────────────────────────────────────────────────────────────
function helixPath(x0, x1, yc, amp, period, phase) {
    const pts = [];
...
function drawDNA(a, st, proc) {
    var _a, _b;
    if (st.opacity <= 0.01)
        return '';
    const span = (_a = a.span) !== null && _a !== void 0 ? _a : [10, 90];
    const x0 = sx(span[0]), x1 = sx(span[1]);
    const yc = sy(((_b = a.at) !== null && _b !== void 0 ? _b : [50, 50])[1]);
 ...
 */
function drawTargetLadder(dna, yc, proc) {
    var _a, _b;
    const seq = ((_a = dna.sequence) !== null && _a !== void 0 ? _a : '').toUpperCase();
    const n = seq.length;
    if (!n)
        return '';
    ...
    for (let i = 0; i < n; i++) {
        ...
        const top = seq[i];
        const bot = (_b = COMPLEMENT[top]) !== null && _b !== void 0 ? _b : '·';
        ...
        if (isMut)
            out.push(mutMarker(colX + dx, botY, proc));
...
function baseTile(x, y, letter, color) {
    return (`<g>` +
...
const mutColor = (proc) => (proc.corrected ? '#16a34a' : '#dc2626');
function mutMarker(x, y, proc) {
...
const guideLetterFor = (dnaBase) => { var _a; return (dnaBase === 'T' ? 'A' : dnaBase === 'A' ? 'U' : (_a = COMPLEMENT[dnaBase]) !== null && _a !== void 0 ? _a : dnaBase); };
/**
 ...
 */
function drawProtein(a, st, proc) {
    var _a, _b;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y);
    const rw = 50 * st.scale, rh = 34 * st.scale;
    const color = (_a = a.color) !== null && _a !== void 0 ? _a : '#2563eb';
    ...
    const label = (_b = a.label) !== null && _b !== void 0 ? _b : '';
    ...
    const tag = label
        ...
        : '';
    return (`<g data-actor="${a.id}" opacity="${o}"${glow}>` +
...
function drawStrand(a, st, proc) {
    var _a, _b;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y);
    const color = (_a = a.color) !== null && _a !== void 0 ? _a : '#ef4444';
    const w = 92 * ((_b = a.size) !== null && _b !== void 0 ? _b : 1);
    ...
    if (a.label)
        out.push(`<text x="${cx}" y="${(cy - 30).toFixed(1)}" text-anchor="middle" font-size="14" font-weight="700" fill="${color}">${esc(a.label)}</text>`);
...
function drawMolecule(a, st) {
    var _a;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y), r = 14 * st.scale, c = (_a = a.color) !== null && _a !== void 0 ? _a : '#10b981';
    return (`<g data-actor="${a.id}" opacity="${st.opacity.toFixed(2)}">` +
...
function drawLabel(a, st) {
    ...
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y), c = (_a = a.color) !== null && _a !== void 0 ? _a : '#7c3aed';
    return (`<g data-actor="${a.id}" opacity="${st.opacity.toFixed(2)}">` +
        `<rect x="${cx - 52}" y="${cy - 17}" width="104" height="32" rx="16" fill="${c}" opacity="0.16"/>` +
        `<text x="${cx}" y="${cy + 5}" text-anchor="middle" font-size="15" font-weight="800" fill="${c}">${esc((_b = a.label) !== null && _b !== void 0 ? _b : '')}</text>` +
        `</g>`);
}
function drawMembrane(a, st) {
    var _a;
    if (st.opacity <= 0.01)
        return '';
    const span = (_a = a.span) !== null && _a !== void 0 ? _a : [10, 90];
    const x0 = sx(span[0]), x1 = sx(span[1]), yc = sy(st.y);
    const heads = [`<g data-actor="${a.id}" opacity="${st.opacity.toFixed(2)}">`];
...
function labelBelow(a, cx, yBelow, color = '#475569') {
    return a.label
        ? `<text x="${cx.toFixed(1)}" y="${yBelow.toFixed(1)}" text-anchor="middle" font-size="14" font-weight="600" fill="${color}">${esc(a.label)}</text>`
        : '';
}
function drawBox(a, st, fx) {
    var _a, _b, _c;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y);
    const w = wpx((_a = a.w) !== null && _a !== void 0 ? _a : 14) * st.scale, h = hpx((_b = a.h) !== null && _b !== void 0 ? _b : 10) * st.scale;
    const c = (_c = a.color) !== null && _c !== void 0 ? _c : '#64748b';
    ...
    if (a.label)
        out.push(`<text x="${cx.toFixed(1)}" y="${(cy + 5).toFixed(1)}" text-anchor="middle" font-size="13" font-weight="700" fill="#ffffff">${esc(a.label)}</text>`);
    out.push('</g>');
    return out.join('');
}
function drawCylinder(a, st, fx) {
    var _a, _b, _c;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y);
    const w = wpx((_a = a.w) !== null && _a !== void 0 ? _a : 26) * st.scale, h = hpx((_b = a.h) !== null && _b !== void 0 ? _b : 12) * st.scale;
    const c = (_c = a.color) !== null && _c !== void 0 ? _c : '#94a3b8';
...
function drawFluid(a, st, t) {
    var _a, _b, _c;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y);
    const w = a.span ? sx(a.span[1]) - sx(a.span[0]) : wpx((_a = a.w) !== null && _a !== void 0 ? _a : 90);
    const h = hpx((_b = a.h) !== null && _b !== void 0 ? _b : 40);
    const x0 = a.span ? sx(a.span[0]) : cx - w / 2;
    const x1 = x0 + w, top = cy - h / 2, bot = cy + h / 2;
    const c = (_c = a.color) !== null && _c !== void 0 ? _c : '#38bdf8';
    // wavy top edge (gently animated)
    const amp = 5, k = 0.03;
    const pts = [`M ${x0.toFixed(1)},${bot.toFixed(1)}`, `L ${x0.toFixed(1)},${top.toFixed(1)}`];
    ...
    if (a.label)
        out.push(`<text x="${(x0 + 16).toFixed(1)}" y="${(top + 22).toFixed(1)}" font-size="14" font-weight="700" fill="${c}">${esc(a.label)}</text>`);
    out.push('</g>');
    return out.join('');
}
function drawGround(a, st) {
    var _a, _b;
    if (st.opacity <= 0.01)
        return '';
    const span = (_a = a.span) !== null && _a !== void 0 ? _a : [4, 96];
    const x0 = sx(span[0]), x1 = sx(span[1]), surf = sy(st.y);
    const c = (_b = a.color) !== null && _b !== void 0 ? _b : '#a16207';
    const out = [`<g data-actor="${a.id}" opacity="${st.opacity.toFixed(2)}">`];
    ...
    if (a.label)
        out.push(`<text x="${(x1 - 12).toFixed(1)}" y="${(surf + 22).toFixed(1)}" text-anchor="end" font-size="13" font-weight="700" fill="#ffffff" opacity="0.85">${esc(a.label)}</text>`);
    out.push('</g>');
    return out.join('');
}
function drawArrow(a, st, fx) {
    var _a, _b;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y);
    const len = wpx((_a = a.w) !== null && _a !== void 0 ? _a : 8) * st.scale;
    const c = (_b = a.color) !== null && _b !== void 0 ? _b : '#334155';
    const rot = fx.rotate; // degrees; 0 = pointing right, 90 = down
    const half = len / 2, head = Math.min(12, len * 0.4);
    const body = `<line x1="${-half}" y1="0" x2="${half - head * 0.6}" y2="0" stroke="${c}" stroke-width="4" stroke-linecap="round"/>` +
    ...
    return (`<g data-actor="${a.id}" opacity="${st.opacity.toFixed(2)}" transform="translate(${cx.toFixed(1)} ${cy.toFixed(1)}) rotate(${rot.toFixed(1)})">` +
        body + lbl + `</g>`);
}
function drawGear(a, st, fx) {
    var _a;
    if (st.opacity <= 0.01)
        return '';
    const cx = sx(st.x), cy = sy(st.y);
    const r = (a.w ? wpx(a.w) / 2 : 22) * st.scale;
    const c = (_a = a.color) !== null && _a !== void 0 ? _a : '#475569';
    const teeth = 10;
    const tooth = [];
    ...
    return (`<g data-actor="${a.id}" opacity="${st.opacity.toFixed(2)}" transform="translate(${cx.toFixed(1)} ${cy.toFixed(1)}) rotate(${fx.rotate.toFixed(1)})">` +
        tooth.join('') +
...
function drawActor(spec, a, t, proc) {
...
// ── Top-level frame ──────────────────────────────────────────────────────────
export function renderFrameSVG(spec, t, opts = {}) {
    ...
    const order = (a) => (BACK.has(a.shape) ? 0 : FRONT.has(a.shape) ? 2 : 1);
    ...
    const chrome = opts.hideChrome ? '' : (`<text x="${VIEW_W / 2}" y="34" text-anchor="middle" font-size="26" font-weight="800" fill="#0f172a">${esc(spec.title)}</text>` +
        ...
        (cap.text
            ...
            : ''));
    return (`<svg viewBox="0 0 ${VIEW_W} ${VIEW_H}" xmlns="http://www.w3.org/2000/svg" width="100%" font-family="Inter, system-ui, sans-serif">` +
        `<defs>` +
...
function renderTooltip(spec, id, t, cam) {
    var _a, _b, _c;
    const a = spec.actors.find((x) => x.id === id);
    if (!a)
        return '';
    const st = actorStateAt(spec, a, t);
    const p = screenOf(st.x, ((_a = a.at) !== null && _a !== void 0 ? _a : [50, 50])[1], cam);
    const w = 268, lines = wrap((_b = a.description) !== null && _b !== void 0 ? _b : '', 32);
    ...
    return (`<g>` +
        ...
        `<text x="${bx + 14}" y="${by + 24}" font-size="15" font-weight="800" fill="#ffffff">${esc((_c = a.label) !== null && _c !== void 0 ? _c : a.id)}</text>` +
        body +
        `</g>`);
}
